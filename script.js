...
const cart= JSON.parse(sessionStorage.getItem("cart")) || [];
    ...
    const grandTotlaEl = document.querySelector(".grand-total");

    cartItemsContainer.innerHTML ="";
    if (cart.length === 0){
        cartItemsContainer.innerHTML="<p>Your cart is empty.</p>";
        subtotalEl.textContent= "0 EGP";
        grandTotlaEl.textContent= "0 EGP"
        return;
    }
    let subtotal=0;
    cart.forEach((item, index) =>{
        const itemTotal= parseFloat(item.price.replace("EGP", "")) * item.quantity;
        subtotal += itemTotal;
        ...
        cartItem.innerHTML=`
         ...
         <div class="product">
            ...
            <span class="total-price">EGP${itemTotal}</span>
    ...
    grandTotlaEl.textContent = `EGP${subtotal.toFixed(2)}`;
}